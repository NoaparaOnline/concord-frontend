...
import { getUsers, ViewSalesManagerManagerAction } from '../../Store/Actions/directorActions';
...
const PrescriptionReport = () => {
    ...
    const getPrescriptionReport = async (id) => {
        ...
        let res = await axios.get(BASEURL + `/reports/prescriptions?child_uid=${id}&from_date=${convertFrom}&to_date=${convertTo}`, {
            headers: {
    ...
    useEffect(() => {
        ...
        dispatch(getUsers(sm?.uid, 'rsm'));
        getPrescriptionReport(sm?.uid)
    }, [dispatch])
